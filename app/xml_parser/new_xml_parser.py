...
with open('./alias-name.csv', 'r') as f:
    ...
    for row in f_csv:
        line += 1
        institute_dict[unicode(row[0])].append((row[2].decode('utf-8'), unicode(row[3])))
    f.close()
        ...
        if name == 'inproceedings' or name == 'article':
            if self.is_required and check_workinfo(self.work):
                ...
                for _alias in _aliases:
                    ...
                    for _institute, _keyword in find_institute(_alias):
                        insert_work(_key, _alias, _type, _title, _year, _institute, _adjustedcount, _ee, _booktitle, _keyword)
            self.is_required = False
...
def insert_work(_key, _alias, _type, _title, _year, _institute, _adjustedcount, _ee, _booktitle, _keyword):
    ...
    from .. import db
    w = Work.query.filter_by(key=_key, keyword=_keyword, alias=_alias).first()
    if w is None:
        work = Work(
